 ...
 * 利用Json比较实体的内容
 */
public class JsonCompareEntity_old {
    ...
    private static void entityContrast(JSONObject beforeSrc, JSONObject afterSrc, List<RelEntity> relEntities, Map<String, Map<String, Object>> resultMap) {
        ...
        for (RelEntity relEntity : relEntities) {
            ...
            Object beforeResult = getValue(beforeSrc, beforeList, 2);
            Object afterResult = getValue(afterSrc, afterList, 2);
            if (beforeResult instanceof JSONArray) {
                for (Object o : (JSONArray) beforeResult) {
                    JSONObject jsonObject = JSON.parseObject(JSONObject.toJSONString(o));
                    System.out.println(jsonObject);
                }
            }
            System.out.println("第一个的值:" + beforeResult);
            System.out.println("第二个的值:" + afterResult);
//            String[] beforeArr = relEntity.getPartyA().split("#");
//            String[] afterArr = relEntity.getPartyB().split("#");
//            Object a = loopData(beforeSrc, beforeArr);
//            System.out.println(a);
//            Object b = loopData(afterSrc, afterArr);
//            System.out.println(b);

        }
    }
    
    private static Object loopData(JSONObject beforeSrc, String[] arr) {
        for (String strArr : arr) {
            String[] beforeStrArr = strArr.split("\\.");
            LinkedList<String> beforeList = arrTurnLinkedList(beforeStrArr);
            Object resultObj = getValue(beforeSrc, beforeList, 2);
            return resultObj;
        }
        return null;
    }
    ...
    //每次递归完都删除一个元素
    private static Object getValue(JSONObject data, LinkedList<String> linkedList, int length) {
        if (data == null || linkedList == null || linkedList.size() == 0) {
            return data;
        }
...
//            根据集合的长度判断是不是集合,是否还有下级
        ...
        getValue(jsonObject, linkedList, length);
        return null;
    }
    ...
    public static void main(String[] args) {
        ...
        compareEntitys(str1, str2, Arrays.asList(relEntity2));
