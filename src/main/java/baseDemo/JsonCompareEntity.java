...
////                        if (afterObject instanceof JSONArray) {
////                            analysisJson(beforeObject, afterObject, beforeLinkedList, afterLinkedList, relEntity, resultList);
////                        } else {
...
//                }
            ...
            if (afterData instanceof JSONArray) {
                //todo 数组的处理  数组长度不相等的情况  数组对比,不能是单纯的下标进行对比,需要给出标志位,若没有标志位,就进行简单的对比
                //当数组长度不一致的时候,取最大的进行循环,缺少的使用null,或者空的属性来补充
                JSONArray beforeJsonArr = (JSONArray) beforeData;
                JSONArray afterJsonArr = (JSONArray) afterData;
                beforeLinkedList.removeFirst();
                afterLinkedList.removeFirst();
                //当数组长度不一致的时候,取最大的进行循环
                if (beforeJsonArr.size() == afterJsonArr.size()) {
                    for (int i = 0; i < beforeJsonArr.size(); i++) {
                        beforeLinkedList.addFirst(beforeKey);
                        afterLinkedList.addFirst(afterKey);
                        Object beforeObject = beforeJsonArr.get(i);
                        Object afterObject = afterJsonArr.get(i);
                        analysisJson(beforeObject, afterObject, beforeLinkedList, afterLinkedList, relEntity, resultList);
                    }
                    //todo 数组越界问题
                } else if (beforeJsonArr.size() > afterJsonArr.size()) {
                    for (int i = 0; i < beforeJsonArr.size(); i++) {
                        beforeLinkedList.addFirst(beforeKey);
                        afterLinkedList.addFirst(afterKey);
                        Object beforeObject = beforeJsonArr.get(i);
...
//                }
    ...
    private static void jsonArrayHandle(JSONArray materJsonArr, JSONArray slaveJsonArr,
        ...
        if (StringUtils.isNotBlank(key)) {
            for (int i = 0; i < materJsonArr.size(); i++) {
                beforeLinkedList.addFirst(beforeKey);
                afterLinkedList.addFirst(afterKey);
                Object beforeObject = materJsonArr.get(i);
                Object afterObject = null;
                String masterKey, slaveKey;
                if (beforeObject instanceof JSONObject) {
                    masterKey = ((JSONObject) beforeObject).getString(key);
                } else {
                    resultList.add("数组比对失败,获取类型不成功,不是一个JSONObject类型!");
                    continue;
                }
                for (int j = 0; j < slaveJsonArr.size(); j++) {
                    afterObject = slaveJsonArr.get(j);
                    //标记
                    if (afterObject instanceof JSONObject) {
                        slaveKey = ((JSONObject) afterObject).getString(key);
                    } else {
                        resultList.add("数组比对失败,获取类型不成功,不是一个JSONObject类型!");
                        continue;
                    }
    ...
    public static void main(String[] args) {
        ...
        RelEntity relEntity2 = new RelEntity("002006.line3.dataorg", "002006的line3名称", "002006.line3.dataorg", "凤金002006的line3名称");
        RelEntity relEntity3 = new RelEntity("002006.line3.dataorg", "名称", "002006.line1.employer", "数据");
//        RelEntity relEntity4 = new RelEntity("002006.line3.dataorg", "名称", "004004.line7.lastmonths", "数据");
        ...
        List<String> list = compareEntitys(str1, str2, Arrays.asList(relEntity1, relEntity2, relEntity3));
        for (String str : list) {
