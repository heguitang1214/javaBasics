...
public class JsonCompareEntity {
    ...
    private static Map<String, Map<String, Object>> compareEntitys(String beforeJson, String afterJson, List<RelEntity> relEntities) {
        //返回值的容器
        Map<String, Map<String, Object>> resultMap = new LinkedHashMap<>();
        JSONObject beforeObject = JSON.parseObject(beforeJson);
        JSONObject afterObject = JSON.parseObject(afterJson);
        entityContrast(beforeObject, afterObject, relEntities, resultMap);
        return resultMap;
    }
     ...
     * 比较json内容
     *
     * @param beforeSrc   json1
     * @param afterSrc    josn2
     * @param relEntities 对应字段映射关系
     */
    private static void entityContrast(JSONObject beforeSrc, JSONObject afterSrc, List<RelEntity> relEntities, Map<String, Map<String, Object>> resultMap) {
        //映射关系字段
        ...
        for (RelEntity relEntity : relEntities) {
            ...
            getValue(beforeSrc, afterSrc, beforeList, afterList, relEntity, resultList, 1);
            System.out.println(resultList);
        }
    }

    //每次递归完都删除一个元素
    private static Object getValue(JSONObject beforeData, JSONObject afterData,
                                   LinkedList<String> befLinkedList, LinkedList<String> aftLinkedList,
                                   RelEntity relEntity, List<String> resultList, int length) {
        //todo  条件判断
        ...
        if (befLinkedList.size() == 0 || aftLinkedList.size() == 0) {
            //对比的两个数组长度不一致
            if (befLinkedList.size() != 0) {
                resultList.add(relEntity.getPartyADesc() + ">>" + relEntity.getPartyBDesc());
                return null;
            }
            if (aftLinkedList.size() != 0) {
                resultList.add(relEntity.getPartyADesc() + ">>" + relEntity.getPartyBDesc());
                return null;
            }
            //基本数据类型
            if (length == 1) {
                String str = beforeData.getString(beforeKey);
                String str1 = afterData.getString(afterKey);
                if (!str.equals(str1)) {
                    resultList.add(relEntity.getPartyADesc() + ">>" + relEntity.getPartyBDesc());
                }
                return null;
                ...
                //数组类型
                JSONArray befortJsonArray = beforeData.getJSONArray(beforeKey);
                JSONArray afterJsonArray = beforeData.getJSONArray(beforeKey);
                for (Object o : befortJsonArray) {
        ...
        JSONObject beforeJson = beforeData.getJSONObject(beforeKey);
        JSONObject afterJson = afterData.getJSONObject(afterKey);
        getValue(beforeJson, afterJson, befLinkedList, aftLinkedList, relEntity, resultList, length);
        return null;
//        if (linkedList.size() == 0) {
////            //最后的元素是属性;最后的元素是数组(集合)   都当做集合来处理
////            //转JSONArray  JSONObject 都会失败
////            String str = data.getString(key);
////            return str;
//
//
////            根据集合的长度判断是不是集合,是否还有下级
//            if (length > 1){
...
//        }
    ...
    //=================================================================================
    private static List<String> compareEntitys1(String beforeJson, String afterJson, List<RelEntity> relEntities) {
        //返回值的容器
        List<String> resultList = new ArrayList<>();
        JSONArray beforeObject = JSONArray.parseArray("[" + beforeJson + "]");
        JSONArray afterObject = JSONArray.parseArray("[" + afterJson + "]");
        entityContrast1(beforeObject, afterObject, relEntities, resultList);
        return resultList;
    }

    private static void entityContrast1(JSONArray beforeSrc, JSONArray afterSrc, List<RelEntity> relEntities, List<String> resultList) {
        //映射关系字段
        for (RelEntity relEntity : relEntities) {
            String[] beforeArr = relEntity.getPartyA().split("#");
            String[] afterArr = relEntity.getPartyB().split("#");
            ...
            for (int i = 0; i < length; i++) {
                String[] before = beforeArr[i].split("\\.");
                String[] after = afterArr[i].split("\\.");
                LinkedList<String> beforeList = arrTurnLinkedList(before);
                LinkedList<String> afterList = arrTurnLinkedList(after);
                getValue1(beforeSrc, afterSrc, beforeList, afterList, relEntity, resultList, length > 1 ? true : false);
            }
//            String[] beforeArr = relEntity.getPartyA().split("\\.");
//            String[] afterArr = relEntity.getPartyB().split("\\.");
            LinkedList<String> beforeList = arrTurnLinkedList(beforeArr);
    ...
    //每次递归完都删除一个元素
    private static void getValue1(JSONArray beforeData, JSONArray afterData,
                                  LinkedList<String> befLinkedList, LinkedList<String> aftLinkedList,
                                  RelEntity relEntity, List<String> resultList, Boolean leafIsArr) {
        //todo  条件判断
        String beforeKey = befLinkedList.getFirst();
        String afterKey = aftLinkedList.getFirst();
        befLinkedList.removeFirst();
        aftLinkedList.removeFirst();
        int size = beforeData.size();
        //todo 转成JSONArray进行递归  到了末尾节点就直接判断了

        //末尾
        if (befLinkedList.size() == 0) {
            String b_str = beforeData.getJSONObject(0).getString(beforeKey);
            String a_str = afterData.getJSONObject(0).getString(afterKey);
            if (!b_str.equals(a_str)) {
                resultList.add(relEntity.getPartyADesc() + "是[" + b_str + "]," + relEntity.getPartyBDesc() + "是[" + a_str + "]");
            }
        } else {
            String b_str = beforeData.getJSONObject(0).getString(beforeKey);
            String a_str = afterData.getJSONObject(0).getString(afterKey);
            ...
            if (leafIsArr) {
                JSONArray jsonArray_b = JSONArray.parseArray(b_str);
                JSONArray jsonArray_a = JSONArray.parseArray(a_str);
                getValue1(jsonArray_b, jsonArray_a, befLinkedList, aftLinkedList, relEntity, resultList, leafIsArr);
            } else {
                JSONArray jsonArray_b = JSONArray.parseArray("[" + b_str + "]");
                JSONArray jsonArray_a = JSONArray.parseArray("[" + a_str + "]");
                getValue1(jsonArray_b, jsonArray_a, befLinkedList, aftLinkedList, relEntity, resultList, leafIsArr);
            }
//            String string = JSONObject.toJSONString(beforeData.getJSONObject(0).getJSONObject(beforeKey));
//            JSONArray jsonObject1 = afterData.getJSONObject(0).getJSONArray(afterKey);
//            getValue1(jsonObject, jsonObject1, befLinkedList, aftLinkedList, relEntity, resultList);
        }


//        for (int i = 0; i < size; i++) {
//            Object object_b = beforeData.get(i);
//            Object object_a = afterData.get(i);
//            Object jsonObject_b = JSONObject.parseObject(JSONObject.toJSONString(object_b)).get(beforeKey);
//            Object jsonObject_a = JSONObject.parseObject(JSONObject.toJSONString(object_b)).get(afterKey);
//
//            String str_b = (String) JSONObject.parseObject(JSONObject.toJSONString(object_b)).get(beforeKey);
//            String str_a = (String) JSONObject.parseObject(JSONObject.toJSONString(object_a)).get(afterKey);
...
//        return;
    ...
    //=================================================================================
    private static void compareEntitys2(String beforeJson, String afterJson, List<RelEntity> relEntities) {
        //返回值的容器
        List<String> resultList = new ArrayList<>();
        JSONObject beforeObject = JSONArray.parseObject(beforeJson);
        JSONObject afterObject = JSONArray.parseObject(afterJson);
        entityContrast2(beforeObject, afterObject, relEntities, resultList);
        for (String str : resultList) {
            System.out.println("结果:" + str);
        }
    }

    private static void entityContrast2(Object beforeSrc, Object afterSrc, List<RelEntity> relEntities, List<String> resultList) {
        //映射关系字段
        for (RelEntity relEntity : relEntities) {
            String[] beforeArr = relEntity.getPartyA().split("#");
            String[] afterArr = relEntity.getPartyB().split("#");
            LinkedList<String> beforeList = arrTurnLinkedList2(beforeArr);
            LinkedList<String> afterList = arrTurnLinkedList2(afterArr);
            getValue2(beforeSrc, afterSrc, beforeList, afterList, relEntity, resultList, false);

        }
    }

    //每次递归完都删除一个元素
    private static void getValue2(Object beforeData, Object afterData,
                                  LinkedList<String> befLinkedList, LinkedList<String> aftLinkedList,
                                  RelEntity relEntity, List<String> resultList, Boolean b) {
        String beforeKey = befLinkedList.getFirst();
        String afterKey = aftLinkedList.getFirst();
//        if (!b){
//            befLinkedList.removeFirst();
...
//                        JSONObject jsonObject_b = ((JSONObject) beforeData).getJSONObject(beforeKey);
//                        JSONObject jsonObject_a = ((JSONObject) afterData).getJSONObject(afterKey);
//                        getValue2(jsonObject_b, jsonObject_a, befLinkedList, aftLinkedList, relEntity, resultList);
                            getValue2(o, o1, befLinkedList, aftLinkedList, relEntity, resultList, false);
                        }else {
...
//                    getValue2(jsonObject_b, jsonObject_a, befLinkedList, aftLinkedList, relEntity, resultList);
            ...
            if (afterData instanceof JSONArray) {
                ...
                for (int i = 0; i < jsonArray_b.size(); i++){
                    ...
                    getValue2(a, bb, befLinkedList, aftLinkedList, relEntity, resultList, true);
                }
//                getValue2(beforeData, afterData, befLinkedList, aftLinkedList, relEntity, resultList);
            } else {
                resultList.add("类型不匹配:" + relEntity.getPartyADesc() + "是[JSONArray]," + relEntity.getPartyBDesc() + "非[JSONArray]");
            }
        } else {
...
//        compareEntitys(str1, str2, Arrays.asList(relEntity1));
        compareEntitys2(str1, str2, Arrays.asList(relEntity1, relEntity2));

//        String relEntity = "001001.line1.reportno#";
//        String[] beforeArr = relEntity.split("#");
...
//        map.put("b", null);
//
//        LocalDate l = (LocalDate) map.get("a");
